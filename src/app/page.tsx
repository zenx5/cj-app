...
export default function Home() {
  const labelText=[
    ...
    "Ingrese su número de cédula, sin guiones ni puntos (Ejemplo: 28821312)",
    "Ingrese su número de télefono (Ejemplo: 04121439101)",
    "Monto cancelado (Ejemplo: 203, 121)",
    "Últimos 4 dígitos de la referencia del pago (Ejemplo: 9821)"
  ]
  return (
    <main className="min-h-screen bg-gradient-to-b from-orange-50 to-amber-50 py-24 px-2">      
      <form className="max-w-2xl mx-auto flex flex-col gap-8">
        ...
        {
          labelText.map((text,index)=><Fieldset key={index} text={text}/>)
        }
        ...
        <fieldset className="flex justify-between text-orange-500 font-semibold text-sm">
          <Link href="./shipment" className="bg-white hover:bg-orange-50 px-6 py-2 shadow-lg border rounded-lg">Siguiente</Link>
